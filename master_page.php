...
// $_SESSION['id_task']=$_GET['id-status'];
$id_task=$_GET['id-status'];
...
<body>
    ...
    <div id=all>
        <div id=left>
            ...
            <div id=main-left>
                <div id=user>
                    <div id=user-name>
                        <div style="padding-left: 5px;padding-ti: 5px;"><img src="mrd.png" style="height:32px;width:32px;margin-top:10px"></div>
                        ...
                        <span id="open"style="margin-top:10px;" >
                            ...
                            <div id="user-list">
                                ...
                                <div class="text-list">
                                    <text><a href="logout.php">Sign Out</a></text>
                                </div>                                
                            </div>
                        </span>
                ...
                echo 
                "<a class=\"left-list-style\"  action=master_page.php>
                <form method=\"post\" class=fake-form action=action.php>
                <input type=text value=".$row["id"]." class=fake-input name=list-id>
                <div class=\"list-icon\"><svg class=\"list rtl-flip\" width=\"20px\" height=\"20px\" viewBox=\"0 0 20 20\" version=\"1.1\" xmlns=\"http://www.w3.org/2000/svg\" xmlns:xlink=\"http://www.w3.org/1999/xlink\"> <g id=\"Web-svgs\" stroke=\"none\" stroke-width=\"1\" fill-rule=\"evenodd\"> <g id=\"list\"> <path d=\"M3,7 C2.44,7 2,6.56 2,6 L2,5 C2,4.44 2.44,4 3,4 L4,4 C4.56,4 5,4.44 5,5 L5,6 C5,6.56 4.56,7 4,7 L3,7 Z M4,5 L3,5 L3,6 L4,6 L4,5 Z M7.5,6 C7.22,6 7,5.78 7,5.5 C7,5.22 7.22,5 7.5,5 L17.5,5 C17.78,5 18,5.22 18,5.5 C18,5.78 17.78,6 17.5,6 L7.5,6 Z M3,12 C2.44,12 2,11.56 2,11 L2,10 C2,9.44 2.44,9 3,9 L4,9 C4.56,9 5,9.44 5,10 L5,11 C5,11.56 4.56,12 4,12 L3,12 Z M4,10 L3,10 L3,11 L4,11 L4,10 Z M7.5,11 C7.22,11 7,10.78 7,10.5 C7,10.22 7.22,10 7.5,10 L17.5,10 C17.78,10 18,10.22 18,10.5 C18,10.78 17.78,11 17.5,11 L7.5,11 Z M3,17 C2.44,17 2,16.56 2,16 L2,15 C2,14.44 2.44,14 3,14 L4,14 C4.56,14 5,14.44 5,15 L5,16 C5,16.56 4.56,17 4,17 L3,17 Z M4,15 L3,15 L3,16 L4,16 L4,15 Z M7.5,16 C7.22,16 7,15.78 7,15.5 C7,15.22 7.22,15 7.5,15 L17.5,15 C17.78,15 18,15.22 18,15.5 C18,15.78 17.78,16 17.5,16 L7.5,16 Z\" id=\"K\"> </path> </g> </g> </svg></div>
                <span class=\"left-tittle\"><button type=\"submit\" class=fake-btn>".$row["list_name"]."</button></span>  
                </form>
                </a>";
                } 
                ?>
        ...
        <div id=conten>
            <div id=head-conten>
                <h1 style="flex:1;padding:10px 14px;font-weight: 200;font-size: 20px;color: #fff;font-weight: 200;">
                <?php
                   $query = "select * from list where id ='$list_id' ";
            ...
            <div id=list-task>
                ...
                <div id=add-task>
                    ...
                    <form class="form-add-task" method="post" action="action.php">
                    <input type="text"  id=met style="width:100%;background:none;border:none;outline:none;height:100%;color:whitesmoke;" placeholder="Add to do..." name="new_task">
                    <button type="submit" class=fake-input></button>
                    <?php
                ...
                <div id="list-add-task" >
                    <div id="popup">
                        ...
                        <div class="context-menu">
                            ...
                            <form method="post" style="display:inline;"  action="action.php">
                                <input type=text name="task_delete" class=fake-input class="task_id" id=list-id >
                            <span class="context-menu-icon"><button class="fake-btn">Delete To-do</button></span>
                            </form>
                        </div>
                    </div>
                ...
                $query = "select * from task where id_list ='$list_id' and status= 'no' ";
                $stmt = $conn->prepare($query);
                foreach($conn->query($query) as $row){
                    echo "<div class=task1>
                    <form method=\"get\" class=\"auto-request\" action=\"master_page.php\"><input type=text value=".$row['id']." class=fake-input name=id-status></form>
                    <div class=\"check-box\"><form method=\"post\" action=action.php><input type=text value=".$row['id']." class=fake-input name=id-status><input type=\"checkbox\"  class=\"checkbox\" onchange=\"this.form.submit()\" name=\"status\" value=yes></form></div>
                    <div id=\"task1\" style=\"flex:1;padding:12px 0\">".$row["task_name"] ."</div><span class=fake-input>".$row['id']."</span>
                    <div style=\"fill:#b4b4b4;padding:12px\"><svg width=\"18px\" height=\"18px\" viewBox=\"0 0 18 18\" version=\"1.1\" xmlns=\"http://www.w3.org/2000/svg\" xmlns:xlink=\"http://www.w3.org/1999/xlink\" xml:space=\"preserve\"> <g> <path d=\"M3.74,18 C3.64,18 3.54,17.96 3.46,17.9 C3.28,17.76 3.2,17.54 3.28,17.34 L5.16,11.5 L0.2,7.9 C0.04,7.78 -0.04,7.56 0.02,7.34 C0.1,7.14 0.28,7 0.5,7 L6.64,7 L8.52,1.16 C8.66,0.76 9.34,0.76 9.48,1.16 L11.38,7 L17.5,7 C17.72,7 17.9,7.14 17.98,7.34 C18.04,7.56 17.96,7.78 17.8,7.9 L12.84,11.5 L14.72,17.34 C14.8,17.54 14.72,17.76 14.54,17.9 C14.38,18.02 14.14,18.02 13.96,17.9 L9,14.3 L4.04,17.9 C3.96,17.96 3.84,18 3.74,18 L3.74,18 Z M9,13.18 C9.1,13.18 9.2,13.2 9.3,13.28 L13.3,16.18 L11.78,11.46 C11.7,11.26 11.78,11.04 11.96,10.92 L15.96,8 L11,8 C10.8,8 10.6,7.86 10.54,7.66 L9,2.94 L7.46,7.66 C7.4,7.86 7.22,8 7,8 L2.04,8 L6.04,10.92 C6.22,11.04 6.3,11.26 6.22,11.46 L4.7,16.18 L8.7,13.28 C8.8,13.2 8.9,13.18 9,13.18 L9,13.18 Z\"></path> </g> </svg></div>
                    </div>";   
                ...
                $stmt = $conn->prepare($query);
                foreach($conn->query($query) as $row){
                    echo "<div class=\"task1 task-complete\">
                    <div class=\"check-box\"><form method=\"post\" action=action.php><input type=text value=".$row['id']." class=fake-input name=id-status><input type=\"checkbox\"  class=\"checkbox\" onchange=\"this.form.submit()\" name=\"status\" value=no checked></form></div>
                    <div id=\"task1\" style=\"flex:1;padding:12px 0\">".$row["task_name"] ."</div><span class=fake-input>".$row['id']."</span>
                    <div style=\"fill:#b4b4b4;padding:12px\"><svg width=\"18px\" height=\"18px\" viewBox=\"0 0 18 18\" version=\"1.1\" xmlns=\"http://www.w3.org/2000/svg\" xmlns:xlink=\"http://www.w3.org/1999/xlink\" xml:space=\"preserve\"> <g> <path d=\"M3.74,18 C3.64,18 3.54,17.96 3.46,17.9 C3.28,17.76 3.2,17.54 3.28,17.34 L5.16,11.5 L0.2,7.9 C0.04,7.78 -0.04,7.56 0.02,7.34 C0.1,7.14 0.28,7 0.5,7 L6.64,7 L8.52,1.16 C8.66,0.76 9.34,0.76 9.48,1.16 L11.38,7 L17.5,7 C17.72,7 17.9,7.14 17.98,7.34 C18.04,7.56 17.96,7.78 17.8,7.9 L12.84,11.5 L14.72,17.34 C14.8,17.54 14.72,17.76 14.54,17.9 C14.38,18.02 14.14,18.02 13.96,17.9 L9,14.3 L4.04,17.9 C3.96,17.96 3.84,18 3.74,18 L3.74,18 Z M9,13.18 C9.1,13.18 9.2,13.2 9.3,13.28 L13.3,16.18 L11.78,11.46 C11.7,11.26 11.78,11.04 11.96,10.92 L15.96,8 L11,8 C10.8,8 10.6,7.86 10.54,7.66 L9,2.94 L7.46,7.66 C7.4,7.86 7.22,8 7,8 L2.04,8 L6.04,10.92 C6.22,11.04 6.3,11.26 6.22,11.46 L4.7,16.18 L8.7,13.28 C8.8,13.2 8.9,13.18 9,13.18 L9,13.18 Z\"></path> </g> </svg></div>
                    </div>";   
                ...
                <div id="popup-complete">
                    ...
                    <div class="context-menu">
                        <span class="context-menu-icon"><svg class="trash" width="20px" height="20px" viewBox="0 0 20 20" version="1.1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink"> <g stroke="none" stroke-width="1" fill-rule="evenodd"> <g id="trash"> <path d="M12,3.5 C12,2.4 11.1,1.5 10,1.5 C8.9,1.5 8,2.4 8,3.5 L5.5,3.5 C4.68,3.5 4,4.18 4,5 L4,7 C4,7.28 4.22,7.5 4.5,7.5 L15.5,7.5 C15.78,7.5 16,7.28 16,7 L16,5 C16,4.18 15.32,3.5 14.5,3.5 L12,3.5 Z M10,2.5 C10.56,2.5 11,2.94 11,3.5 L9,3.5 C9,2.94 9.44,2.5 10,2.5 L10,2.5 Z M15,6.5 L5,6.5 L5,5 C5,4.72 5.22,4.5 5.5,4.5 L14.5,4.5 C14.78,4.5 15,4.72 15,5 L15,6.5 Z M14.5,8.5 C14.22,8.5 14,8.72 14,9 L14,16 C14,16.28 13.78,16.5 13.5,16.5 L6.5,16.5 C6.22,16.5 6,16.28 6,16 L6,9 C6,8.72 5.78,8.5 5.5,8.5 C5.22,8.5 5,8.72 5,9 L5,16 C5,16.82 5.68,17.5 6.5,17.5 L13.5,17.5 C14.32,17.5 15,16.82 15,16 L15,9 C15,8.72 14.78,8.5 14.5,8.5 L14.5,8.5 Z M9,9 C9,8.72 8.78,8.5 8.5,8.5 C8.22,8.5 8,8.72 8,9 L8,15 C8,15.28 8.22,15.5 8.5,15.5 C8.78,15.5 9,15.28 9,15 L9,9 Z M12,9 C12,8.72 11.78,8.5 11.5,8.5 C11.22,8.5 11,8.72 11,9 L11,15 C11,15.28 11.22,15.5 11.5,15.5 C11.78,15.5 12,15.28 12,15 L12,9 Z" id="j"></path> </g> </g> </svg></span>
                        <span class="label"><button>Delete To-do</button></span>             
                    </div>
                </div>
            </div>
        </div>
        <div id=right>
            <div id=head-right>
                <div style="padding:17px"><input type="checkbox" style="width:20px;height:20px"></div>
                <div style="flex:1">
                    <form style="display:inline;" method="post" action="action.php">
                        <input class="fake-right-tip fake-input" type=text class=fake-input name=id_task_change>
                        <input id="rightip" type="text" style="width:100%;height:99%;border:none;outline:none" onchange="this.form.submit()" name=name_task_change>
                    </form>
                </div>
            ...
            <div id=right-list>
                <div class=right-task>
                    ...
                    <div class="right-input">
                        <form  style="display:inline;" method="post" action="action.php">
                        <input class="fake-right-tip fake-input"  type=text  name="id-task-date" >
                        <input class=date_sumit  type="date" style="width:100%;height:100%;border:none;outline:none;" placeholder="Set due date"  name="task_date" onchange="this.form.submit()" value="<?php
                ...
                <div class=right-task>
                    ...
                    <div class="right-input"><input type="text" style="width:100%;height:100%;border:none;outline:none" placeholder="Remind me"></div>
                </div>
                ...
                <div class="sub-task-drop">
                    ...
                    $stmt = $conn->prepare($query);
                    foreach($conn->query($query) as $row){
                        echo"<div class=sub-task>
                        <form method=post action=action.php class=sub-task-form>
                        <div class=\"row-15 sub-task-checkbox\"><input type=checkbox ></div>
                        <div class=row-70><input type=\"text\"class=input-sub-task value=".$row['sub_task_name']." name=new_sub_task onchange=\"this.form.submit()\"></div>
                        <div class=row-15><i class=\"fas fa-times sub-task-icon\"></i></div>
                        </form>
                    </div>";
                    }
                    ?>
                ...
                <div class="right-icon-special"><svg class="plus-small" width="20px" height="20px" viewBox="0 0 20 20" version="1.1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" xml:space="preserve"> <g> <path d="M10,10l0,6.5c-0.003,0.053 -0.008,0.103 -0.024,0.155c-0.038,0.116 -0.12,0.217 -0.226,0.278c-0.047,0.027 -0.094,0.042 -0.146,0.056c-0.052,0.008 -0.051,0.008 -0.104,0.011c-0.053,-0.003 -0.103,-0.008 -0.155,-0.024c-0.15,-0.05 -0.271,-0.171 -0.321,-0.321c-0.016,-0.052 -0.021,-0.102 -0.024,-0.155l0,-6.5l-6.5,0c-0.046,-0.002 -0.058,-0.001 -0.104,-0.011c-0.103,-0.022 -0.197,-0.076 -0.268,-0.154c-0.169,-0.188 -0.169,-0.482 0,-0.67c0.035,-0.038 0.077,-0.072 0.122,-0.098c0.078,-0.045 0.161,-0.062 0.25,-0.067l6.5,0l0,-6.5c0.005,-0.089 0.022,-0.172 0.067,-0.25c0.126,-0.219 0.406,-0.31 0.636,-0.207c0.048,0.022 0.093,0.05 0.132,0.085c0.078,0.071 0.132,0.165 0.154,0.268c0.01,0.046 0.009,0.058 0.011,0.104l0,6.5l6.5,0c0.046,0.002 0.058,0.001 0.104,0.011c0.103,0.022 0.197,0.076 0.268,0.154c0.169,0.188 0.169,0.482 0,0.67c-0.035,0.038 -0.077,0.072 -0.122,0.098c-0.078,0.045 -0.161,0.062 -0.25,0.067l-6.5,0Z"></path> </g> </svg></div>
                <div class="right-input-special">
                    <form method="post" action="action.php">
                    <input type="text" class="input-add-sub-task" style="width:100%;height:100%;border:none;outline:none" placeholder="Add a sub task" name=sub-task onchange="this.form.submit()">
                    <input class="fake-right-tip fake-input" type=text class=fake-input name=id_task_change>
                    </form>
                </div>
                ...
                $stmt = $conn->prepare($query);
                foreach($conn->query($query) as $row){
                    echo "<form method=post action=action.php class=note-form><input type=\"text\" style=\"width:100%;height:100%;border:none;outline:none\" onchange=\"this.form.submit()\"  name=task_note placeholder=\"Add a note...\" value=".$row["note"]."></form>";
                }
                ?>
                </div>
                ...
                <div class=right-task>
                    ...
                    <div class="right-input">
                        <form action="action.php" method="post" enctype="multipart/form-data" class=fake-form-file>
                        <input type=text  class="fake-right-tip fake-input" name=id_task>
                        <input type="file"  class="upload"  name="file_upload"   onchange="this.form.submit()">
                        </form> 
                        <div class=add-file>Add a file</div>              
                    </div>
                ...
                <div id=file_drop>
                    ...
                    $stmt = $conn->prepare($query);
                    foreach($conn->query($query) as $row){
                        echo "<div class=file><div class=file-icon>jgp</div><form method=post class=file-form action=show.php><input type=text value=".$row['id']." class=fake-input name=file_id><input type=submit class=file-input value=".$row['link']."></form></div>";
                    }
                    ?>
                </div>
            ...
            <div id=comment>
                <div style="margin: 10px 10px 0 10px;"><input type="text" style="width:100%;height:80%;" placeholder="Add a comment..."></div>
            </div>
            <div id=detail>
    ...
    <script src="jquery.js?v=21" language="javascript"></script>
